...
class VIMAEnvBase(gym.Env):
    ...
    def __init__(
        ...
        assets_root = os.path.join(os.path.dirname(os.path.dirname(__file__)), "tasks", "assets")
        ...
        for modality in self.modalities:
            if modality == "rgb":
                obs_cam_space = {
                    view: gym.spaces.Box(0, 255, shape=(3, *config["image_size"]), dtype=np.uint8)
                ...
                obs_cam_space = {
                    view: gym.spaces.Box(0, 255, shape=config["image_size"], dtype=np.uint8)
                    for view, config in self.agent_cams.items()
                }
                obs_space.update(
                    {
    ...
    def set_task(self, task: BaseTask | str | None = None, task_kwargs: dict | None = None):
        # setup task
        ...
        self.position_bounds = gym.spaces.Box(
            ...
            high=np.array([0.75, 0.5], dtype=np.float32),
            shape=(2,),
            dtype=np.float32,
        )
    ...
    def get_prompt_and_assets(self):
        ...
        Intentionally make this method to preserve Gym API. Otherwise something like `initial_obs,
        prompt, p_assets = env.reset()` breaks Gym API.
        """
    ...
    def reset(self, workspace_only=False):
        ...
        assert len(self.obj_id_reverse_mapping) > 0, (
            "Please add the mapping into the dict when reset new tasksUse the method"
            " add_object_id_reverse_mapping_info in pybullet_utils.py, which is similar to the"
            " add_any_object method.Hint: You could also refer to the novel_concept_grounding task"
            " as an example to see how to implement that"
        )
        ...
        assert all(
            ...
            "Incomplete object_id mapping dict. Please check whether there are missing objects"
            " that are not added into the dictCurrently, we have"
            f" {len(self.obj_id_reverse_mapping.keys())} in dict, but the total n_objects is"
            f" {sum(len(v) for v in self.obj_ids.values())}[Debug Info] Matching Truth Table:"
            f" {[(obj_id in self.obj_id_reverse_mapping) for each_catagory in self.obj_ids.values() for obj_id in each_catagory]}Mapping"
            f" Dict {self.obj_id_reverse_mapping}Object IDs {self.obj_ids}"
        )
        ...
        if isinstance(self.ee, Suction):  # we use MoveEndEffector Primitive
            ...
            sub_actions = [
                ...
                # sub-action 2, vertically raise the arm to a safe height before move to the place position
                {
                    ...
                    "rotation": np.array(utils.eulerXYZ_to_quatXYZW((0, 0, 0)), dtype=np.float32),
                    "release": False,
                    "push": False,
                },
    ...
    def oracle_step(self, oracle_action: dict):
        """Execute oracle action. Note that the original "Pick and Place" primitive can be divided
        into several sub-actions using "Move End Effector" primitive.
    ...
    def step_simulation(self):
        ...
        if (
            ...
            and self.step_counter % self.task.constraint_checking["checking_sim_interval"] == 0
        ):
