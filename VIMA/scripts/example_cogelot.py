...
def main(cfg):
    ...
    assert cfg.task in get_partition_to_specs()["test"][cfg.partition]
    ...
    env = TimeLimitWrapper(
        ResetFaultToleranceWrapper(
            make(
                cfg.task,
                modalities=["segm", "rgb"],
                task_kwargs=get_partition_to_specs()["test"][cfg.partition][cfg.task],
                seed=seed,
                render_prompt=False,
    ...
    while True:
        ...
        while True:
            ...
            try:
                print("obs_tokens_to_forward", obs_tokens_to_forward.shape)
                print(any_stack(obs_tokens_this_env, dim=0).shape)
                print()
            except:
                pass
            try:
                print("action_tokens_to_forward", action_tokens_to_forward.shape)
            except:
                pass
            try:
            ...
            try:
                ...
                pass
            try:
                print("obs_masks_to_forward", obs_masks_to_forward.shape)
            except:
                pass
            ...
            elapsed_steps += 1
            img_num = len(os.listdir('output_imgs'))
            print("step taken", actions)
            print("done", done, "img_num", img_num, "elapsed_steps", elapsed_steps)
            if done:
                set_trace()
            cv2.imwrite(f"output_imgs/{img_num}_{prompt.replace(' ', '_')}.png", np.transpose(obs['rgb']['front'], (1, 2, 0)))
            if done or elapsed_steps > 10:
                break
